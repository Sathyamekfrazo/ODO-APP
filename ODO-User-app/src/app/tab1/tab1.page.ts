...
export class Tab1Page {
  ...
  localUrl: any;
  vehcleNovali:boolean=false;
  vehcleName:boolean=false;
  vehcleReference:boolean=false;
  ...
  prePayload(event){
    ...
    console.log(body);
    localStorage.setItem('postDataaddvehicle',JSON.stringify(body));
  ...
  validateNo(e): boolean {
    ...
    if (charCode > 31 && (charCode < 48 || charCode > 57)) {
      console.log('false')
      this.vehcleNovali=true;
      return false
    }
    console.log('true')
    this.vehcleNovali=false;
    return true
}
vehicleName(e){
  ...
  if (regex.test(str)) {
    this.vehcleName=false;
      return true;
  }
  else
  {
  e.preventDefault();
  this.vehcleName=true;
...
vehicleNameref(e){
  ...
  var str = String.fromCharCode(!e.charCode ? e.which : e.charCode);
  if (regex.test(str)) {
    this.vehcleReference=false;
      return true;
  }
  else
  {
  e.preventDefault();
  this.vehcleReference=true;
  // alert('Please Enter Alphabate');
  return false;
  }
}
